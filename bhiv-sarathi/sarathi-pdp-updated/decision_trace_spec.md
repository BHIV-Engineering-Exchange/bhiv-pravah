...
**Tamper detection:** If any trace is modified, its `trace_hash` changes, which breaks the `prev_trace_hash` link of the next trace. Detection is O(1) per trace. The engine explicitly runs a `VerifyChain()` sequence upon startup/closing to recompute every hash and validate the entire chain integrity.
...
| **Replay Isolation** | During deterministic replay tests (`ReplayMode = true`), trace emissions are computed and validated in memory only, preventing side-effect mutations to the persistent governance chain. |
