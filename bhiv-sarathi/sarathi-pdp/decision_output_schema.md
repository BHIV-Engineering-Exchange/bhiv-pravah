...
SHA-256 hex digest of the canonical JSON of the rules array (6 required fields only: rule_id, agent_role, resource_type, action, classification_max, verdict). Computed at policy load time. Verified against stored hash to detect tampering.
...
ISO-8601 UTC with microsecond precision. Records when the PDP rendered the verdict. Used for audit sequencing and TTL calculation.
