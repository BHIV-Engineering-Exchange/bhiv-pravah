...
**Generated:** 2026-04-14T08:57:38Z
...
| TKLC-05 | 3 | TOKEN_LIFECYCLE | Revoke token enforcement_hash and verify subsequent executio... | NEW_TOKEN_INDEPENDENT | first_token=12c0f13b-5c51-4439-aae5-157569428895 second_state=EXECUTION_PERMITTED | `REVOCATION_SCOPED_CORRECTLY` | ✅ |
| TKLC-06 | 3 | TOKEN_LIFECYCLE | Unicode/null-byte injection in agent_id — zero-width space... | BLOCKED | EXECUTION_BLOCKED | `NO_TOKEN: execution requires a valid capability token` | ✅ |
...
| RPSA-01 | 8 | RESOURCE_PARSING_SYSTEM_ATTACK | Path Traversal Bypass — agent attempts to access core-engi... | DENY | <nil> | `DENY` | ❌ |
| RPSA-02 | 8 | RESOURCE_PARSING_SYSTEM_ATTACK | Case Sensitivity Escalation — agent_id with uppercase lett... | DENY | <nil> | `DENY` | ❌ |
...
| `UNIQUENESS_true` | 1 |
| `NO_TOKEN: execution requires a valid capability token` | 4 |
| `PERMIT=20 BLOCK=0` | 1 |
| `ORDER_INDEPENDENT` | 1 |
| `TTL_WINDOW_VALID` | 1 |
...
- WEAKNESS: RPSA-01 (Path Traversal Bypass — agent attempts to access core-engine via path traversal) — expected=DENY actual=<nil> error=DENY
- WEAKNESS: RPSA-02 (Case Sensitivity Escalation — agent_id with uppercase letters) — expected=DENY actual=<nil> error=DENY
