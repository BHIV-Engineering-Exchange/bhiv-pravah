...
def _send(payload: dict) -> None:
    ...
    try:
        resp = _req.post(PRAVAH_URL, json=payload, headers=headers, timeout=10)
        if resp.status_code != 200:
            logger.error(f"Pravah send failed with status {resp.status_code}: {resp.text}")
    except Exception as e:
        logger.error(f"Pravah send exception: {e}")
