"""
verify_gurukul_observability.py
================================
Standalone verification script — proves that Pravah can observe the Gurukul
backend runtime WITHOUT owning or interfering with it.

What this script does:
  1. Constructs a SSPL-signed telemetry payload with app = "gurukul-backend"
  ...
  4. Exits cleanly — no side effects on the running Gurukul server

Prerequisites:
  - Pravah Control Plane must be running on port 7000
    (start it with: python start_with_observer.py  OR
                    cd pravah-bhiv/backend && python wsgi.py)
...
def build_signed_payload(trace_id: str, state: str = "running", latency_ms: float = 0.0) -> tuple[dict, dict]:
    """Construct and sign a Pravah runtime telemetry payload for gurukul-backend."""
    payload = {
        "app": APP_NAME,
        "env": "dev",
        "state": state,
    ...
    # Canonical serialisation (matches pravah_adapter.py signature logic)
...
def main():
    trace_id = f"gurukul-verify-{uuid.uuid4().hex[:12]}"

    print("=" * 65)
    print("  Pravah Observer — Gurukul Backend Observability Verification")
    print("=" * 65)
    ...
    payload, headers = build_signed_payload(
        trace_id=trace_id,
        state="running",
        latency_ms=42.0,  # synthetic — simulates a healthy Gurukul request
    )
    ...
    if resp.status_code in (200, 201, 202):
        ...
        print(f"        * gurukul-backend service card should show status")
        print("        * Open http://localhost:7000/api/control-plane/history/gurukul-backend")
