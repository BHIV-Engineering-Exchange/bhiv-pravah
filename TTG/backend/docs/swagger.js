...
const options = {
  definition: {
    ...
    info: {
      ...
      version: '1.0.0',
      description: [
        '## Real-Time Micro-Bridge + Multi-Agent Orchestrator',
        '',
    ...
    tags: [
      ...
      { name: 'TTG — Compile & Assets', description: 'Text-to-Game: compile text → gameplay contract + engine capabilities' },
      { name: 'Core Execution', description: 'Submit and track execution schemas through the engine pipeline' },
      { name: 'Game State', description: 'Live game state, sessions, and deterministic replay' },
      { name: 'Execution Interface', description: 'Phase 4 hardened contract entry point (engineExecutionContract_v3)' },
      { name: 'Pipeline', description: 'Maritime governance pipeline — run, replay, artifacts, telemetry' },
      { name: 'Simulation', description: 'Deterministic SumScript simulation engine — run, replay, results' },
      { name: 'TTS', description: 'Text-to-speech audio generation' },
      { name: 'Socket.IO — Dashboard', description: '⚡ Default namespace `/`. Connect with Bearer JWT. Handles presence, actions, agents, heartbeat, job status.' },
      { name: 'Socket.IO — Engine', description: '⚡ Namespace `/engine`. Engine authenticates with engine JWT. Handles job dispatch, telemetry, game lifecycle, runtime events.' },
      { name: 'Socket.IO — Sim Stream', description: '⚡ Namespace `/simulate/stream`. Real-time simulation tick streaming and replay.' },
    ],
    components: {
      securitySchemes: {
      ...
      schemas: {
        ...
        GameplayContract: {
          ...
          properties: {
            game_mode: { type: 'string', enum: ['runner', 'sidescroller', 'arena'] },
            movement: { type: 'object', properties: { speed: { type: 'number', example: 5 } } },
            spawn_rules: {
              type: 'object',
              properties: {
                obstacles: { type: 'integer', example: 3 },
                frequency: { type: 'number', example: 2.0 },
                distance: { type: 'number', example: 10.0 },
              },
            },
            player_params: {
              type: 'object',
              properties: {
                ...
                jetpack: { type: 'boolean', example: false },
                jump_height: { type: 'number', example: 5 },
              },
            },
            physics: {
              type: 'object',
              properties: {
                gravity: { type: 'number', example: -9.8 },
                friction: { type: 'number', example: 0.1 },
                bounce: { type: 'number', example: 0.0 },
                air_resistance: { type: 'number', example: 0.05 },
                collision_force: { type: 'number', example: 1.0 },
              },
            },
            score_rules: { type: 'object', example: { distance: 1, collectibles: 10 } },
            end_conditions: { type: 'array', items: { type: 'string' }, example: ['collision'] },
          },
        },
        ExecutionSchema: {
          type: 'object',
          required: ['execution_id', 'trace_id', 'executionSchema'],
          properties: {
            execution_id: { type: 'string', example: 'exec_001' },
            trace_id: { type: 'string', example: 'trace_001' },
            user_id: { type: 'string', example: 'user_01' },
            timestamp: { type: 'number', example: 1700000000000 },
            intent: { type: 'string', example: 'fast runner with obstacles' },
            executionSchema: { $ref: '#/components/schemas/GameplayContract' },
          },
        },
        JobStatus: {
          type: 'object',
          properties: {
            ...
            jobType: {
              type: 'string',
              enum: ['BUILD_SCENE', 'SPAWN_ENTITY', 'SPAWN_PLAYER', 'SPAWN_OBSTACLE_SYSTEM',
                'SPAWN_ENEMIES', 'SPAWN_PLATFORMS', 'SPAWN_PICKUPS', 'START_LOOP', 'START_GAME', 'STOP_GAME'],
            },
            status: { type: 'string', enum: ['queued', 'dispatched', 'running', 'completed', 'failed'] },
    ...
    paths: {
            ...
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  required: ['userId'],
                  properties: { userId: { type: 'string', example: 'admin' } },
                },
              },
            },
          },
              ...
              content: {
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      token: { type: 'string' },
                      engineId: { type: 'string', example: 'atharva_engine_01' },
                      expiresIn: { type: 'string', example: '365 days' },
                      usage: { type: 'string' },
                    },
                  },
                },
              ...
              content: {
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      token: { type: 'string' },
                      userId: { type: 'string', example: 'user_static_demo' },
                      expiresIn: { type: 'string', example: '365 days' },
                      usage: { type: 'string' },
                    },
                  },
      ...
      // ══════════════════════════════════════════════
      // TTG — COMPILE & GAMEPLAY ASSETS
      // ══════════════════════════════════════════════

      '/api/intent/compile': {
        post: {
          tags: ['TTG — Compile & Assets'],
          summary: 'Compile natural language text → gameplay contract',
          description: [
            'Converts user text into a full gameplay contract.',
            ...
            content: {
              'application/json': {
                schema: {
                  ...
                  required: ['text'],
                  properties: {
                    text: { type: 'string', example: 'fast runner game with obstacles and jetpack' },
                    userId: { type: 'string', example: 'user_01' },
                  },
                },
                examples: {
                  runner: { summary: 'Runner', value: { text: 'fast runner game with obstacles', userId: 'user_01' } },
                  platformer: { summary: 'Platformer', value: { text: 'easy platformer with jump and coins', userId: 'user_01' } },
                  arena: { summary: 'Arena', value: { text: 'hard arena survival with enemies', userId: 'user_01' } },
                },
              },
            },
          },
                  ...
                  schema: {
                    ...
                    properties: {
                      ...
                      intent: {
                        type: 'object',
                        properties: {
                          genre: { type: 'string', enum: ['runner', 'sidescroller'] },
                          pacing: { type: 'string', enum: ['slow', 'medium', 'fast'] },
                          difficulty: { type: 'string', enum: ['easy', 'medium', 'hard'] },
                          abilities: { type: 'array', items: { type: 'string' }, example: ['jump', 'jetpack'] },
                          obstacles: { type: 'boolean' },
                          pickups: { type: 'boolean' },
                        },
                      },
                      schema: { $ref: '#/components/schemas/GameplayContract' },
                      message: { type: 'string', example: 'Compiled runner game' },
                    },
                  },
                },
              },
            },
            400: { description: 'Validation failed or contract violation' },
                  ...
                  schema: {
                    ...
                    properties: {
                      ...
                      features: {
                        type: 'object',
                        properties: {
                          genres: { type: 'array', items: { type: 'string' }, example: ['runner', 'sidescroller'] },
                          abilities: { type: 'array', items: { type: 'string' }, example: ['jump', 'dash', 'jetpack'] },
                          difficulty: { type: 'array', items: { type: 'string' }, example: ['easy', 'medium', 'hard'] },
                          pacing: { type: 'array', items: { type: 'string' }, example: ['slow', 'medium', 'fast'] },
                        },
                      },
                    },
                  },
                  ...
                  schema: {
                    ...
                    properties: {
                      entities: {
                        ...
                        example: ['player', 'enemy', 'obstacle', 'obstacle_spawner', 'pickup', 'ground', 'platform', 'spawner', 'checkpoint'],
                      },
                      components: {
                        type: 'array', items: { type: 'string' },
                        example: ['runner_controller', 'platformer_controller', 'arena_controller', 'ai_controller', 'collider', 'mesh', 'rigidbody', 'health', 'trigger'],
                      },
                      jobs: {
                        type: 'array', items: { type: 'string' },
                        example: ['BUILD_SCENE', 'SPAWN_ENTITY', 'SPAWN_PLAYER', 'SPAWN_OBSTACLE_SYSTEM', 'SPAWN_ENEMIES', 'SPAWN_PLATFORMS', 'SPAWN_PICKUPS', 'START_LOOP'],
                      },
                    },
                  },
                },
                      ...
                      gameplay_assets: {
                        ...
                        properties: {
                          runner: {
                            type: 'object',
                            properties: {
                              template_id: { type: 'string', example: 'runner_v1' },
                              entities: { type: 'array', items: { type: 'string' }, example: ['player', 'ground', 'obstacle_spawner'] },
                              components: { type: 'object', example: { player: ['runner_controller', 'collider'] } },
                              jobs: { type: 'array', items: { type: 'string' }, example: ['BUILD_SCENE', 'SPAWN_PLAYER', 'SPAWN_OBSTACLE_SYSTEM', 'START_LOOP'] },
                              defaults: { type: 'object', example: { movement_speed: 5, spawn_frequency: 3, gravity: -9.8 } },
                            },
                          },
                          platformer: { type: 'object', description: 'Platformer template (same shape as runner)' },
                          arena: { type: 'object', description: 'Arena template (same shape as runner)' },
                        },
                      },
                    },
                  },
      ...
      '/api/intent/runtime-consumption': {
        get: {
          ...
          parameters: [
            {
              ...
              in: 'query',
              required: false,
              schema: { type: 'string', enum: ['runner', 'platformer', 'arena'] },
          ...
          responses: {
            ...
            400: { description: 'Unknown game_mode value' },
          },
        },
      },
      ...
      '/core/execute': {
        post: {
          ...
          description: [
            'Main execution entry point. Validates signature + nonce, then dispatches through:',
          ...
          requestBody: {
            ...
            content: {
              'application/json': {
                schema: { $ref: '#/components/schemas/ExecutionSchema' },
              },
            },
          },
          responses: {
            ...
            401: { description: 'Security validation failed — bad signature, expired nonce, or timestamp skew' },
            500: { description: 'Server error' },
          },
        },
      },
      ...
      '/core/execute-from-text': {
        post: {
            ...
            content: {
              'application/json': {
                schema: {
                  ...
                  required: ['prompt'],
                  properties: {
                    prompt: { type: 'string', example: 'Create a fast runner game in a desert with obstacles' },
                    user_id: { type: 'string', example: 'user_01' },
                  },
                },
              },
          ...
          responses: {
            200: { description: 'Prompt compiled and execution queued' },
            400: { description: 'Missing or invalid prompt' },
            500: { description: 'Prompt Runner error' },
          },
        },
      },
            ...
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  required: ['prompt'],
                  properties: { prompt: { type: 'string', example: 'runner game in a volcano' } },
                },
              },
            },
          },
          responses: {
            200: { description: 'Compiled executionSchema (not dispatched)' },
                ...
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      ...
                      url: { type: 'string', example: 'http://127.0.0.1:8001' },
                    },
                  },
                },
              },
      ...
      '/core/execute-from-prompt': {
        post: {
          ...
          responses: { 403: { description: 'Direct prompt execution not allowed' } },
        },
      },
      ...
      '/core/execution/{id}': {
        get: {
          ...
          parameters: [
            { name: 'id', in: 'path', required: true, schema: { type: 'string' }, example: 'exec_001' },
            { name: 'detailed', in: 'query', schema: { type: 'string', enum: ['true', 'false'] }, description: 'true = include full jobDetails array' },
          ],
          responses: {
            200: {
                    ...
                    properties: {
                      ...
                      execution: {
                        ...
                        properties: {
                          execution_id: { type: 'string' },
                          trace_id: { type: 'string' },
                          status: { type: 'string', enum: ['received', 'running', 'completed', 'failed'] },
      ...
      '/core/executions': {
        get: {
          ...
          parameters: [
            { name: 'status', in: 'query', schema: { type: 'string', enum: ['received', 'running', 'completed', 'failed'] } },
          ],
          responses: { 200: { description: 'Array of execution summaries' } },
        },
      },
      ...
      '/core/telemetry/{id}': {
        get: {
          ...
          responses: {
            200: { description: 'Telemetry events array' },
            404: { description: 'No telemetry found' },
          },
        },
      },
      ...
      '/core/telemetry': {
        get: {
          ...
          responses: { 200: { description: 'All telemetry events' } },
        },
      },
      ...
      '/core/game-sessions': {
        get: {
          ...
          responses: { 200: { description: 'Array of active session IDs' } },
        },
      },
      ...
      '/core/game-state/{sessionId}/replay': {
        get: {
          ...
          parameters: [{ name: 'sessionId', in: 'path', required: true, schema: { type: 'string' } }],
          responses: {
            200: { description: 'Reconstructed state' },
            500: { description: 'Replay error' },
          },
        },
      },

      // ══════════════════════════════════════════════
      // EXECUTION INTERFACE (hardened v3)
      // ══════════════════════════════════════════════
      ...
      '/execute': {
        post: {
          ...
          description: [
            'Strict validation: headers AND body fields must match.',
          ...
          parameters: [
            ...
            { name: 'X-Execution-Id', in: 'header', required: true, schema: { type: 'string' } },
          ],
          requestBody: {
            required: true,
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  required: ['execution_id', 'trace_id', 'game_mode', 'entities', 'physics', 'scoring'],
                  properties: {
                    execution_id: { type: 'string' },
                    trace_id: { type: 'string' },
                    game_mode: { type: 'string', enum: ['runner', 'sidescroller', 'open_scene'] },
                    entities: { type: 'array', minItems: 1, items: { type: 'object' } },
                    physics: {
                      type: 'object',
                      required: ['gravity'],
                      properties: { gravity: { type: 'array', items: { type: 'number' }, example: [0, -9.8, 0] } },
                    },
                    scoring: {
                      type: 'object',
                      required: ['rules'],
          ...
          responses: {
            ...
            400: { description: 'Rejected — missing headers, field mismatch, invalid game_mode, empty entities, missing gravity or scoring.rules' },
          },
        },
      },
            ...
            content: {
              'application/json': {
                schema: {
                  ...
                  required: ['text'],
                  properties: {
                    text: { type: 'string', example: 'Game started. Good luck!' },
                    language: { type: 'string', default: 'en', example: 'en', description: 'BCP-47 code: en, hi, fr, de, etc.' },
                  },
                },
              },
            },
      ...
      '/pipeline/run': {
        post: {
          ...
          requestBody: {
            ...
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  required: ['vessel_id', 'lat', 'lon', 'speed', 'heading'],
                  properties: {
                    vessel_id: { type: 'string', example: 'vessel_001' },
                    lat: { type: 'number', example: 25.0 },
                    lon: { type: 'number', example: 55.0 },
          ...
          responses: {
            200: { description: 'Pipeline result with decision ALLOW/BLOCK/FLAG' },
            400: { description: 'Missing required fields' },
            422: { description: 'Enforcement failure' },
            500: { description: 'Pipeline crash' },
          },
        },
      },
      ...
      '/pipeline/result/{trace_id}': {
        get: {
          ...
          responses: {
            200: { description: 'All artifacts keyed by type' },
            404: { description: 'No artifacts found' },
            500: { description: 'Artifact read error' },
          },
        },
      },
              ...
              content: {
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      success: { type: 'boolean' },
                      service: { type: 'string', example: 'pipeline' },
                      status: { type: 'string', example: 'ok' },
                      bucket_accessible: { type: 'boolean' },
      ...
      '/pipeline/telemetry/{trace_id}': {
        get: {
          ...
          parameters: [
            ...
            { name: 'stage', in: 'query', schema: { type: 'string' }, description: 'Filter by stage name (e.g. decision_received)' },
          ],
          responses: {
            200: { description: 'Telemetry events for the trace' },
            400: { description: 'Unknown stage filter' },
            404: { description: 'No telemetry found' },
            500: { description: 'Trace consistency violation' },
          },
        },
      },
      ...
      '/simulate/run': {
        post: {
          ...
          description: [
            'Validates against simulationContract.v1, adapts to SumScript, runs ticks.',
          ...
          requestBody: {
            ...
            content: {
              'application/json': {
                schema: {
                  type: 'object',
                  required: ['trace_id', 'execution_id', 'domain', 'scenario', 'entities', 'behaviors', 'rules', 'constraints'],
                  properties: {
                    trace_id: { type: 'string', example: 'trace_sim_001' },
                    execution_id: { type: 'string', example: 'exec_sim_001' },
                    domain: { type: 'string', example: 'runner' },
                    scenario: { type: 'string', example: 'obstacle_course' },
                    entities: { type: 'array', items: { type: 'object' } },
                    behaviors: { type: 'array', items: { type: 'object' } },
                    rules: { type: 'array', items: { type: 'object' } },
                    constraints: { type: 'object' },
                    ticks: { type: 'integer', default: 10, example: 10 },
                  },
                },
              },
            },
          ...
          responses: {
            200: { description: 'Simulation completed — returns simulationState.v1' },
            413: { description: 'Request body exceeds 256KB limit' },
            422: { description: 'Contract validation failed or simulation error' },
          },
        },
      },
      ...
      '/simulate/replay/{trace_id}': {
        post: {
          ...
          parameters: [{ name: 'trace_id', in: 'path', required: true, schema: { type: 'string' } }],
          responses: {
            200: { description: 'Replay result matching original run' },
            400: { description: 'trace_id required' },
            422: { description: 'Replay failed or determinism violation' },
          },
        },
      },
      ...
      '/simulate/result/{trace_id}': {
        get: {
          ...
          parameters: [{ name: 'trace_id', in: 'path', required: true, schema: { type: 'string' } }],
          responses: {
            200: { description: 'simulationState.v1 result object' },
            404: { description: 'No result found for trace_id' },
          },
        },
      },
              ...
              content: {
                'application/json': {
                  schema: {
                    ...
                    properties: {
                      status: { type: 'string', example: 'ok' },
                      node: { type: 'string', example: 'simulation' },
                      headless: { type: 'boolean', example: true },
                      ui_required: { type: 'boolean', example: false },
      ...
      // ══════════════════════════════════════════════
      // SOCKET.IO — DASHBOARD (default namespace /)
      // ══════════════════════════════════════════════

      '/socket.io/dashboard': {
        get: {
          tags: ['Socket.IO — Dashboard'],
          ...
          responses: { 200: { description: 'WebSocket upgrade — see description for event protocol' } },
        },
      },

      // ══════════════════════════════════════════════
      // SOCKET.IO — ENGINE (/engine namespace)
      // ══════════════════════════════════════════════

      '/socket.io/engine': {
        get: {
          tags: ['Socket.IO — Engine'],
          summary: '⚡ Namespace `/engine` — engine authenticates with engine JWT',
          description: [
            '**Connect:** `io("http://localhost:3000/engine", { auth: { token: engineToken } })`',
      ...
      // ══════════════════════════════════════════════
      // SOCKET.IO — SIM STREAM (/simulate/stream)
      // ══════════════════════════════════════════════

      '/socket.io/simulate-stream': {
        get: {
          tags: ['Socket.IO — Sim Stream'],
          summary: '⚡ Namespace `/simulate/stream` — real-time simulation tick streaming',
          description: [
            '**Connect:** `io("http://localhost:3000/simulate/stream")`',
            '',
            '### Client → Server events',
            '| Event | Payload | Description |',
            '|---|---|---|',
