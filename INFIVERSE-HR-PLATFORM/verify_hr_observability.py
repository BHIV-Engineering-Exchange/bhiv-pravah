"""
verify_hr_observability.py
===========================
Standalone verification script — proves that Pravah can observe the
INFIVERSE-HR-PLATFORM gateway runtime WITHOUT owning or interfering with it.

What this script does:
  1. Constructs a SSPL-signed telemetry payload with app = "infiverse-hr-platform"
  ...
  4. Exits cleanly — no side effects on the running HR gateway
...
Usage:
  python verify_hr_observability.py
"""
...
APP_NAME = "infiverse-hr-platform"
...
def build_signed_payload(trace_id: str, state: str = "running", latency_ms: float = 0.0) -> tuple[dict, dict]:
    """Construct and sign a Pravah runtime telemetry payload for infiverse-hr-platform."""
    payload = {
        "app": APP_NAME,
        "env": "dev",
        "state": state,
...
def main():
    trace_id = f"hr-verify-{uuid.uuid4().hex[:12]}"

    print("=" * 65)
    print("  Pravah Observer -- INFIVERSE HR Platform Observability Verify")
    print("=" * 65)
    ...
    payload, headers = build_signed_payload(
        trace_id=trace_id,
        state="running",
        latency_ms=55.0,  # synthetic -- simulates a healthy HR gateway request
    )
    ...
    if resp.status_code in (200, 201, 202):
        ...
        print(f"        * infiverse-hr-platform service card should show status")
        print("        * Open http://localhost:7000/api/control-plane/history/infiverse-hr-platform")
