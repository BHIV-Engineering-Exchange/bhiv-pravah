...
pravah_adapter.py
==================
Pravah Bhiv observation adapter for INFIVERSE-HR-PLATFORM.
...
Usage (import anywhere in the gateway):
    from pravah_adapter import emit_hr_signal

    emit_hr_signal(state="running", latency_ms=95.0)
    emit_hr_signal(state="error", errors_last_min=3)
...
def _send(payload: dict) -> None:
    ...
    trace_id = f"hr-{uuid.uuid4().hex[:16]}"
    ...
    try:
        ...
        # Pravah observer must NEVER impact the HR platform's normal operation.
...
def emit_hr_signal(
    ...
    and has zero impact on the HR platform's response time or reliability.
...
def start_heartbeat(interval_seconds: int = 60) -> None:
    ...
    def _loop():
        while True:
            ...
            emit_hr_signal(state="running")

    t = threading.Thread(target=_loop, daemon=True, name="pravah-hr-heartbeat")
    t.start()
    logger.info(
        "[Pravah] HR heartbeat started — emitting every %ds to %s",
