...
try:
    import sys as _sys
    import os as _os
    _gateway_dir = _os.path.dirname(_os.path.dirname(__file__))
    if _gateway_dir not in _sys.path:
        _sys.path.insert(0, _gateway_dir)
    from pravah_adapter import start_heartbeat as _pravah_start_heartbeat
    _pravah_start_heartbeat(interval_seconds=60)
    print("[Pravah] INFIVERSE HR Platform heartbeat active — observing, not owning.")
