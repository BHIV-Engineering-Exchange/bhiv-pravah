#!/usr/bin/env node
/**
 * verify_blackhole_observability.js
 * ===========================
 * Standalone verification script — proves that Pravah can observe the
 * workflow-blackhole runtime WITHOUT owning or interfering with it.
 */
...
function main() {
    const traceId = `workflow-verify-${crypto.randomUUID().replace(/-/g, "").substring(0, 12)}`;

    console.log("=".repeat(70));
    console.log("  Pravah Observer -- Workflow Blackhole Node.js Observability Verification");
    console.log("=".repeat(70));
