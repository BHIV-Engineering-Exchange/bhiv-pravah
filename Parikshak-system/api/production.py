...
async def apply_human_override(request: HumanOverrideRequest, current_user: dict = Depends(require_reviewer_or_governor)):
    ...
    try:
        # Replay protection check MUST run first to prevent replay attacks regardless of case state
        sig_token = request.signature or f"token-override-{request.case_id}-{request.approval_version or 1}"
        ...
        if isinstance(review.analysis, dict) and "engineering_review" in review.analysis:
            review.analysis["engineering_review"]["governance_state"] = decision_val
