...
def setup_test_traces():
    ...
    yield
    # Cleanup only test traces created by this module
    for test_dir in ["test-trace-ready", "test-trace-needs-review", "test-trace-rejected"]:
        path = os.path.join(TRACES_DIR, test_dir)
