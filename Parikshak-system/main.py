...
# ── Pravah Bhiv Observation Layer ─────────────────────────────────────────────
# Pravah observes — not owns — the execution of this system.
# This block starts a background heartbeat that sends signed telemetry to the
# Pravah Control Plane every 60 seconds. Fire-and-forget: swallows all errors,
# never blocks, never affects request handling.
try:
    import sys as _sys
    import os as _os
    _observability_dir = _os.path.dirname(__file__)
    if _observability_dir not in _sys.path:
        _sys.path.insert(0, _observability_dir)
    from observability.pravah_adapter import start_heartbeat as _pravah_start_heartbeat
    _pravah_start_heartbeat(interval_seconds=60)
    print("[Pravah] Parikshak System heartbeat active — observing, not owning.")
except Exception as _pravah_exc:
    print(f"[Pravah] Observer not available (non-critical): {_pravah_exc}")
# ──────────────────────────────────────────────────────────────────────────────
