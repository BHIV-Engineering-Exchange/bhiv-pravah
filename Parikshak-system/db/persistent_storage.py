...
class ProductStorage:
    ...
    def store_submission(self, submission: TaskSubmission) -> None:
        ...
        try:
            ...
            if isinstance(self._save_nolock, Mock):
                is_mocked = True
        except Exception:
            pass

        if self.use_db and not is_mocked:
            db = SessionLocal()
            try:
    ...
    def store_next_task(self, next_task: NextTaskRecord) -> None:
        ...
        if self.use_db and not is_mocked:
            ...
            try:
                ...
                rev = db.query(ReviewModel).filter(ReviewModel.submission_id == next_task.previous_submission_id).first()
                if rev and rev.review_state == "PENDING_REVIEW":
                    # Defer assignment creation to human approval flow
                    return
    ...
    def _is_mocked(self) -> bool:
        try:
            from unittest.mock import Mock
            return isinstance(self._save_nolock, Mock)
        except Exception:
            return False
    ...
    def get_submission(self, submission_id: str) -> Optional[TaskSubmission]:
        if self.use_db and not self._is_mocked():
            try:
                db = SessionLocal()
