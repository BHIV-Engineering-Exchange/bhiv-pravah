    ...
    def log_execution(self, output: Dict[str, Any], is_hard_reject: bool = False):
        ...
        with self._lock:
            try:
                ...
                # 3. Emit Pravah Telemetry Signal (observe only, fire-and-forget)
                try:
                    from observability.pravah_adapter import emit_pravah_signal
                    
                    # Map evaluation status to Pravah state
                    pravah_state = "running"
                    if is_hard_reject:
                        pravah_state = "crashed"
                    elif output.get("evaluation_result") == "FAIL":
                        pravah_state = "degraded"
                    
                    # Read latency (evaluation_time_ms) if present, default to 0.0
                    latency = 0.0
                    if isinstance(output.get("meta"), dict):
                        latency = float(output["meta"].get("evaluation_time_ms", 0.0))
                    
                    emit_pravah_signal(
                        state=pravah_state,
                        latency_ms=latency,
                        errors_last_min=1 if (is_hard_reject or output.get("evaluation_result") == "FAIL") else 0,
                        extra={
                            "trace_id": output.get("trace_id", "unknown-trace"),
                            "submission_id": output.get("submission_id", "unknown-submission"),
                        }
                    )
