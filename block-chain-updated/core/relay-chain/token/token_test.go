...
func TestNewToken(t *testing.T) {
	t.Run("Valid Initialization", func(t *testing.T) {
		...
		// NewToken starts with totalSupply=0; initialSupply sets maxSupply cap
		assert.Equal(t, uint64(0), tk.TotalSupply())
		assert.Equal(t, uint64(1000), tk.MaxSupply())
	})
}
...
func TestBurn(t *testing.T) {
	// Use unlimited max supply (0) so Mint is not capped
	tk := NewToken("Test", "TST", 18, 0)
	...
	t.Run("Burn valid amount", func(t *testing.T) {
		...
		// Verify total supply (500 - 300 = 200)
		assert.Equal(t, uint64(200), tk.TotalSupply())
