...
func (t *Token) Approve(owner, spender string, amount uint64) error {
	...
	// Structured logging — pass allowance directly to avoid re-acquiring the lock.
	LogTokenOperation(t, "approve", owner, spender, amount, "success", map[string]interface{}{
		"tx_hash":         t.generateTxHash("approval", owner+":"+spender, amount),
		"allowance_after": amount,
	})
