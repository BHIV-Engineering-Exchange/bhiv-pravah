...
func (t *Token) Transfer(from, to string, amount uint64) error {
	...
	if t.balances[from] < amount {
		...
		LogTokenOperation(t, "transfer", from, to, amount, "failed", map[string]interface{}{"error": err.Error()})
		return err
	}
	...
	// Structured logging — pass balances directly to avoid re-acquiring the lock.
	LogTokenOperation(t, "transfer", from, to, amount, "success", map[string]interface{}{
		"tx_hash":             t.generateTxHash("transfer", from+":"+to, amount),
		"from_balance_after":  t.balances[from],
		"to_balance_after":    t.balances[to],
	})
