...
func NewNode(ctx context.Context, port int) (*Node, error) {
	// In Docker mode listen on all interfaces so peer containers can connect.
	// Locally keep 127.0.0.1 to avoid link-local (169.254.x.x) binding failures.
	bindIP := "127.0.0.1"
	if os.Getenv("DOCKER_MODE") == "true" || os.Getenv("BLOCKCHAIN_DOCKER") == "true" {
		bindIP = "0.0.0.0"
	}
	listenAddr := fmt.Sprintf("/ip4/%s/tcp/%d", bindIP, port)
