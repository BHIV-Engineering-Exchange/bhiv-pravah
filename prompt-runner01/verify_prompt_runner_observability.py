#!/usr/bin/env python3
"""
verify_prompt_runner_observability.py
======================================
Standalone verification script to validate that Pravah can observe the
prompt-runner01 runtime without interfering with it.

What this script does:
  1. Constructs a signed telemetry payload with app = "prompt-runner01"
...
# Configuration
PRAVAH_URL = os.getenv("PRAVAH_URL", "http://127.0.0.1:7000/api/runtime")
...
def build_signed_payload(trace_id: str, state: str = "running", latency_ms: float = 0.0) -> tuple[dict, dict]:
    """Construct and sign a Pravah runtime telemetry payload for prompt-runner01."""
    payload = {
        "app": APP_NAME,
        "env": "dev",
        "state": state,
...
def main():
    trace_id = f"prompt-runner-verify-{uuid.uuid4().hex[:12]}"

    print("=" * 70)
    print("  Pravah Observer -- Prompt Runner 01 Observability Verification")
    print("=" * 70)
