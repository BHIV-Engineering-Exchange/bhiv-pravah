#!/usr/bin/env python3
"""
verify_uniguru_observability.py
================================
Standalone verification script to validate that Pravah can observe the
uniguru_ai runtime without interfering with it.

What this script does:
  1. Constructs a signed telemetry payload with app = "uniguru_ai"
...
def build_signed_payload(trace_id: str, state: str = "running", latency_ms: float = 0.0) -> tuple[dict, dict]:
    """Construct and sign a Pravah telemetry payload for uniguru_ai."""
    payload = {
        "app": APP_NAME,
        "env": "dev",
        "state": state,
...
def main():
    trace_id = f"uniguru-verify-{uuid.uuid4().hex[:12]}"

    print("=" * 70)
    print("  Pravah Observer -- UniGuru AI Observability Verification")
    print("=" * 70)
