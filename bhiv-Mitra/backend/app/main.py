...
# -------------------------------------------------
# Pravah Telemetry Adapter
# -------------------------------------------------
try:
    from app.observability.pravah_adapter import emit_pravah_signal, start_heartbeat
    _PRAVAH_AVAILABLE = True
except ImportError:
    _PRAVAH_AVAILABLE = False

# -------------------------------------------------
# Initialize Monitoring (OpenTelemetry + Prometheus + Pravah)
# -------------------------------------------------
...
async def _startup_monitoring():
    ...
    if _PRAVAH_AVAILABLE:
        ...
        logger.info("[Pravah] Telemetry observer started for bhiv-Mitra")

@app.middleware("http")
async def pravah_telemetry_middleware(request: Request, call_next):
    import time
    start_time = time.time()
    try:
        response = await call_next(request)
        latency_ms = (time.time() - start_time) * 1000.0
        if _PRAVAH_AVAILABLE and not request.url.path.startswith("/metrics"):
