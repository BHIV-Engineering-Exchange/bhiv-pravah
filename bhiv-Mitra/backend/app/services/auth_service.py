...
class AuthService:
    ...
    def _activate_fallback(self, exc: Exception) -> None:
        """In production, always raise. In dev, allow in-memory fallback with warning."""
        if self._is_production():
            logger.error("Auth service MongoDB unavailable in PRODUCTION: %s. Raising.", exc)
            raise exc
        if self._runtime_fallback_enabled or not self._can_fallback():
            raise exc
        self._runtime_fallback_enabled = True
        logger.warning("Auth service switching to in-memory store (DEV ONLY): %s", exc)
